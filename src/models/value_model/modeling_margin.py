...
class ValueFunctionModuleMargin(pl.LightningModule):
    def __init__(
        self,
        input_dim,
        hidden_dim,
    ...
    def shared_step(self, batch, stage):
        ...
        last_indices_pref = pref_masks.float().argmax(dim=1, keepdim=True)
        last_indices_pref[pref_masks.sum(dim=1) == 0] = -1
        last_indices_rej = rej_masks.float().argmax(dim=1, keepdim=True)
        last_indices_rej[rej_masks.sum(dim=1) == 0] = -1
        batch_idx = torch.arange(batch_size)
        final_preds_pref = predictions_pref[batch_idx, last_indices_pref.squeeze()]
        final_preds_rej = predictions_rej[batch_idx, last_indices_rej.squeeze()]
        target = torch.ones_like(final_preds_pref)

        margin_loss = F.margin_ranking_loss(
            final_preds_pref, final_preds_rej, target, margin=0
        )

        total_loss = (pairwise_loss + final_loss + margin_loss) / batch_size
